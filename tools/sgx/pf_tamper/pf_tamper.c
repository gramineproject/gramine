...
static void tamper_truncate(void) {
    ...
    truncate_file("trunc_meta_plain_7", FIELD_TRUNCATED(metadata_plaintext_header_t, metadata_gmac));
    ...
    truncate_file("trunc_meta_enc_3", mdps + FIELD_TRUNCATED(metadata_decrypted_header_t, file_size));
    truncate_file("trunc_meta_enc_4", mdps + offsetof(metadata_decrypted_header_t, root_mht_node_key));
    truncate_file("trunc_meta_enc_5", mdps + FIELD_TRUNCATED(metadata_decrypted_header_t, root_mht_node_key));
    truncate_file("trunc_meta_enc_6", mdps + offsetof(metadata_decrypted_header_t, root_mht_node_gmac));
    truncate_file("trunc_meta_enc_7", mdps + FIELD_TRUNCATED(metadata_decrypted_header_t, root_mht_node_gmac));
    truncate_file("trunc_meta_enc_8", mdps + offsetof(metadata_decrypted_header_t, file_data));
    truncate_file("trunc_meta_enc_9", mdps + FIELD_TRUNCATED(metadata_decrypted_header_t, file_data));
...
#define __BREAK_PF(suffix, ...) do { \
    ...
    pf_decrypt(mht_enc, sizeof(*mht_enc), &meta_dec->root_mht_node_key, &meta_dec->root_mht_node_gmac, mht_dec, \
               "mht"); \
    __VA_ARGS__ \
...
#define BREAK_MHT(suffix, ...) do { \
    __BREAK_PF(suffix, __VA_ARGS__ { \
                   pf_encrypt(mht_dec, sizeof(*mht_dec), &meta_dec->root_mht_node_key, &meta_dec->root_mht_node_gmac, \
                              mht_enc, "mht"); \
               } ); \
} while (0)
