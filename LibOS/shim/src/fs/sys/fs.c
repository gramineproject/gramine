...
BEGIN_CP_FUNC(numa_topology) {
    ...
    if (!off) {
        uint64_t num_nodes_online = g_pal_control->topo_info.nodes.resource_count;
        size_t numa_topo_sz = num_nodes_online * sizeof(PAL_NUMA_TOPO_INFO);
...
BEGIN_CP_FUNC(cache) {
    ...
    size_t off = GET_FROM_CP_MAP(obj);
    if (!off) {
        uint64_t num_cache_lvls = g_pal_control->topo_info.num_cache_index;
        size_t cache_topo_sz = num_cache_lvls * sizeof(PAL_CORE_CACHE_INFO);
...
BEGIN_CP_FUNC(core_topology) {
    ...
    size_t off = GET_FROM_CP_MAP(obj);
    if (!off) {
        uint64_t num_cores_online = g_pal_control->topo_info.online_logical_cores.resource_count;
        size_t core_topo_sz = num_cores_online * sizeof(PAL_CORE_TOPO_INFO);
...
BEGIN_RS_FUNC(topo_info) {
    ...
    g_pal_control->topo_info.num_sockets = topo_info->num_sockets;
    g_pal_control->topo_info.physical_cores_per_socket = topo_info->physical_cores_per_socket;
