...
static bool __update_attr(PAL_STREAM_ATTR* attr, int level, int optname, char* optval) {
    ...
    if (level == SOL_SOCKET) {
        switch (optname) {
             ...
             * convertion in both cases (SO_RCVTIMEO and SO_SNDTIMEO); probably add these checks
             * in shim_do_setsockopt and shim_do_getsockopt instead of this internal func.
