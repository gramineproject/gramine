 ...
 * is this even possible in Gramine? */
long shim_do_connect(int fd, void* addr, int _addrlen) {
    ...
    struct sockaddr_storage* addr_st = addr;
    if (sock->state == SOCK_CONNECTED && addrlen >= sizeof(addr_st->ss_family)
            && addr_st->ss_family == AF_UNSPEC) {
