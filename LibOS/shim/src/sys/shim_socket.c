...
static int unix_copy_addr(struct sockaddr* saddr, const char* path) {
    ...
    /* for simplicity, we return not the dynamically-calculated minimally needed size but a static
     * max size */
    return sizeof(struct sockaddr_un);
}
