...
int check_and_load_exec(struct shim_handle** exec, const char* path,
    ...
    const char** in_argv = argv;
    assert(*out_new_argv == NULL);
    ...
    while (true) {
        ...
        if ((ret = open_executable(file, in_path)) < 0) {
            goto err;
        }
        ...
        if((ret = check_and_load_shebang(file, &interp_path, in_argv, &new_argv)) < 0) {
            goto err;
        }
        if (in_path != path)
            free((void*)in_path);
...
int check_and_load_shebang(struct shim_handle* file, const char** out_interp_path,
    ...
    if ((ret < 0) || (shebang_len <= prefix_len) || (shebang[0] != '#') || (shebang[1] != '!')) {
    ...
    shebang_len -= prefix_len;
    memmove(&shebang[0], &shebang[prefix_len], shebang_len);
    ...
    log_debug("Total memory required to hold string args %lu, Total number of args (array elements) %lu", new_argv_total_bytes, new_argv_size);

    new_argv = calloc((new_argv_size + 1), sizeof(char*));
    if (!new_argv) {
        ret = -ENOMEM;
    ...
    for (const char** a = argv_shebang; *a; a++) {
        ...
        log_debug("Printing arguments from shebang %s", new_argv[new_argv_idx]);
        new_argv_idx++;
        argv_cur += size;
    }
    ...
    for (const char** a = argv; *a; a++) {
        ...
        memcpy(argv_cur, *a, size);
        new_argv[new_argv_idx] = argv_cur;
        log_debug("Printing arguments from argv %s", new_argv[new_argv_idx]);
        new_argv_idx++;
        argv_cur += size;
    }
    ...
    *out_interp_path = *new_argv;
    ...
    log_debug("Interpreter to be used %s", *out_interp_path);
