...
struct link_map {
     ...
     *
     * Note that `l_diff` name is a departure from the ELF standard and Glibc code: the ELF standard
     * uses the term "Base Address" and Glibc uses `l_addr`. We find ELF/Glibc terms misleading and
     * therefore replace them with `l_diff`. For the context, below is the exerpt from the ELF spec,
     * Section "Program Header", Sub-section "Base Address":
     *
...
static struct link_map* map_elf_object(struct shim_handle* file, ElfW(Ehdr)* ehdr) {
    ...
    if (ehdr->e_type == ET_DYN) {
         ...
         * ensure that l_diff will not be less than 0.
         */
        void* addr;
