...
#define SHEBANG_MIN_SIZE 2   /* Size of shebang starting sequence (#!) */
...
int check_and_load_exec(struct shim_handle** exec, const char* path, const char** argv, const char*** argv_n) {
    ...
    while (1) {
        ...
        if ((ret = check_elf_object(file)) < 0) {
            ...
            if((ret = check_and_load_shebang(file, &interp_path, INTERP_PATH_SIZE, argv, argv_n)) < 0) {
                put_handle(file);
                return ret;
            }
            path = interp_path;
...
int check_and_load_shebang(struct shim_handle* file, char** interp_path, size_t interp_path_size, const char** argv, const char*** argv_n) {
    ...
    if ((ret < 0) || (shebang_size <= SHEBANG_MIN_SIZE) || (shebang[0] != '#') || (shebang[1] != '!')) {
    ...
    shebang_size -= SHEBANG_MIN_SIZE;
    memmove(&shebang[0], &shebang[SHEBANG_MIN_SIZE], shebang_size);
