...
int check_and_load_shebang(struct shim_handle* file, const char** out_interp_path,
    ...
    log_debug("size of args %lu, array size %lu", new_argv_total_bytes, new_argv_size);
    ...
    for (const char** a = argv_shebang; *a; a++) {
        ...
        log_debug("Printing arguments from script %s", new_argv[new_argv_idx]);
        new_argv_idx++;
        argv_cur += size;
    }
    ...
    for (const char** a = argv; *a; a++) {
        ...
        memcpy(argv_cur, *a, size);
        new_argv[new_argv_idx] = argv_cur;
        log_debug("Printing arguments from execve %s", new_argv[new_argv_idx]);
        new_argv_idx++;
        argv_cur += size;
    }
