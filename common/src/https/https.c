...
static int parse_url(const char *ipaddr, char *src_url, HTTP_URL *urlinfo);
...
static int recv_cb(void* hdl, unsigned char* buf, size_t buf_size) {
    ...
    int ret = DkStreamRead(hdl_ssl_fd, 0, &sz, (void*)buf, NULL, 0);
    if (ret < 0) {
...
static int send_cb(void* hdl, const unsigned char* buf, size_t buf_size) {
    ...
    log_debug("send_cb: %ld", buf_size);
    int ret = DkStreamWrite(hdl_ssl_fd, 0, &sz, (void*)buf, NULL);
    if (ret < 0) {
        if (ret == -EINTR || ret == -EAGAIN || ret == -EWOULDBLOCK)
...
static int parse_url(const char *ipaddr, char *src_url, HTTP_URL *urlinfo)
{
    ...
    else
    {
        ...
        snprintf(urlinfo->path, 256, "%s", p2);
    }
    log_debug("parse_url:3");
    if((p1=strstr_alt(str, ":")) != NULL)
    {
        *p1=0;
        snprintf(urlinfo->host, 256, "%s", str);
        snprintf(urlinfo->port, 5, "%s", p1+1);
    }
    else
    {
        snprintf(urlinfo->host, 256, "%s", str);
    ...
    log_debug("parse url: host:%s, port:%s, url:%s", urlinfo->host, urlinfo->port, urlinfo->path);
    return 0;
...
static void mbedtls_pal_disconnect(HTTP_INFO *hi)
{
    if (hi->tls.handle_ssl_fd) {
        DkObjectClose(hi->tls.handle_ssl_fd);
...
static int mbedtls_pal_connect(HTTP_INFO *hi, const char *host, const char *port)
{
    ...
    ret = DkStreamOpen(uri, PAL_ACCESS_RDWR, 0, PAL_CREATE_IGNORED, 0,
    ...
    if (ret >= 0 && hi->tls.handle_ssl_fd) {
        ...
        mbedtls_pal_disconnect(hi);
        log_error("mbedtls_pal_disconnect failed: %d", ret);
    }
    log_debug("mbedtls_pal_connect");
    return ret;
}
