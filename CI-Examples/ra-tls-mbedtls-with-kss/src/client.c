...
static char g_expected_isv_ext_prod_id[16];
static char g_expected_isv_family_id[16];
static char g_expected_config_id[64];
static char g_expected_config_svn[2];
...
static bool g_verify_isv_ext_prod_id = false;
static bool g_verify_isv_family_id   = false;
static bool g_verify_config_id       = false;
static bool g_verify_config_svn      = false;

/* RA-TLS: our own callback to verify SGX measurements including KSS values */
static int my_verify_measurements(const char* mrenclave, const char* mrsigner,
    ...
    assert(isv_ext_prod_id && isv_family_id && config_id && config_svn);
    ...
    if (g_verify_isv_ext_prod_id &&
            memcmp(isv_ext_prod_id, g_expected_isv_ext_prod_id, sizeof(g_expected_isv_ext_prod_id)))
        return -1;

    if (g_verify_isv_family_id &&
            memcmp(isv_family_id, g_expected_isv_family_id, sizeof(g_expected_isv_family_id)))
        return -1;

    if (g_verify_config_id &&
            memcmp(config_id, g_expected_config_id, sizeof(g_expected_config_id)))
        return -1;

    if (g_verify_config_svn &&
            memcmp(config_svn, g_expected_config_svn, sizeof(g_expected_config_svn)))
        return -1;
...
int main(int argc, char** argv) {
    ...
    if (ra_tls_verify_lib) {
        ...
        ra_tls_set_measurement_callback_f = dlsym(ra_tls_verify_lib, "ra_tls_set_measurement_including_kss_callback");
        if ((error = dlerror()) != NULL) {
            mbedtls_printf("%s\n", error);
            return 1;
        }
    ...
    if (argc > 2 && ra_tls_verify_lib) {
        if (argc != 10) {
            mbedtls_printf("USAGE: %s %s <expected mrenclave> <expected mrsigner>"
                           ...
                           " <expected isv_ext_prod_id> <expected isv_faimily_id>"
                           " <expected config_id> <expected config_svn>"
        ...
        g_verify_isv_ext_prod_id = true;
        g_verify_isv_family_id   = true;
        g_verify_config_id       = true;
        g_verify_config_svn      = true;
        ...
        if (!strcmp(argv[6], "0")) {
            mbedtls_printf("  - ignoring ISV_EXT_PROD_ID\n");
            g_verify_isv_ext_prod_id = false;
        } else if (parse_hex(argv[6], g_expected_isv_ext_prod_id, sizeof(g_expected_isv_ext_prod_id)) < 0) {
            mbedtls_printf("Cannot parse ISV_EXT_PROD_ID!\n");
            return 1;
        }

        if (!strcmp(argv[7], "0")) {
            mbedtls_printf("  - ignoring ISV_FAMILY_ID\n");
            g_verify_isv_family_id = false;
        } else if (parse_hex(argv[7], g_expected_isv_family_id, sizeof(g_expected_isv_family_id)) < 0) {
            mbedtls_printf("Cannot parse ISV_FAMILY_ID!\n");
            return 1;
        }

        if (!strcmp(argv[8], "0")) {
            mbedtls_printf("  - ignoring CONFIG_ID\n");
            g_verify_config_id = false;
        } else if (parse_hex(argv[8], g_expected_config_id, sizeof(g_expected_config_id)) < 0) {
            mbedtls_printf("Cannot parse CONFIG_ID!\n");
            return 1;
        }

        if (!strcmp(argv[9], "0")) {
            mbedtls_printf("  - ignoring CONFIG_SVN\n");
            g_verify_config_svn = false;
        } else {
            errno = 0;
            uint16_t config_svn = (uint16_t)strtoul(argv[9], NULL, 10);
            if (errno) {
                mbedtls_printf("Cannot parse CONFIG_SVN\n");
                return 1;
            }
            memcpy(g_expected_config_svn, &config_svn, sizeof(config_svn));
        }
