...
static int init_amx_random(void) {
    syscall(SYS_arch_prctl, /*ARCH_REQ_XCOMP_PERM*/0x1023, /*AMX_TILEDATA*/18);
...
int main(int argc, char** argv) {
    ...
    if (argc < 2 || (strcmp(argv[1], "eexit") && strcmp(argv[1], "aex"))) {
        printf("Run: %s eexit|aex [inuse]\n", argv[0]);
        return 1;
    }
    ...
    if (strcmp(argv[1], "aex") == 0) {
        /* special case of testing AEX/ERESUME flow -- the logic is implemented completely in
         * Gramine codebase, our program only tells Gramine when to trigger the logic (by issuing a
         * sched_yield() syscall) and then the program goes into an infinite loop of a #UD exception
         * to invoke AEX/ERESUME -- Gramine's PAL is supposed to terminate after some time */
        printf("Starting AEX/ERESUME micro-benchmark... ");
        ret = sched_yield();
        if (ret) {
            printf("sched_yield failed?!\n");
            return ret;
        }
        asm __volatile__("ud2"); /* enclave will AEX/ERESUME on this instruction forever */
        return 0; /* unreachable */
    }

    assert(strcmp(argv[1], "eexit") == 0);
    printf("Starting EXIT/EENTER micro-benchmark... ");
