...
_MESON_DEVENV = _os.getenv('MESON_DEVENV') == '1'

if (__version__.startswith('@')
and not _os.getenv('GRAMINE_IMPORT_FOR_SPHINX_ANYWAY') == '1'
and not _MESON_DEVENV):
    raise RuntimeError(
        'You are attempting to run the tools from repo, without installing. '
        'Please install Gramine before running Python tools, or use `meson devenv`. See '
