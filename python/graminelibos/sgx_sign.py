...
def sign_with_file(key, passphrase):
...
def sign_with_local_key(data, key, passphrase=None):
    ...
    with key:
        private_key = serialization.load_pem_private_key(
            key.read(), password=passphrase, backend=_cryptography_backend)

    public_numbers = private_key.public_key().public_numbers()

    if public_numbers.e != SGX_RSA_PUBLIC_EXPONENT:
        print(
            f'invalid RSA key: expected exponent {SGX_RSA_PUBLIC_EXPONENT}, got {public_numbers.e}',
            file=sys.stderr)
        sys.exit(1)
