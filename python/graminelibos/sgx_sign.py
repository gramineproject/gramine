...
def get_loadcmds(elf_filename):
    with open(elf_filename, 'rb') as file:
        for seg in elftools.elf.elffile.ELFFile(file):
            if seg.p_type != 'PT_LOAD':
                continue
            yield seg.p_offset, seg.p_vaddr, seg.p_filesz, seg.p_memsz, seg.p_flags
...
class MemoryArea:
    ...
    def __init__(self, desc, elf_filename=None, content=None, addr=None, size=None,
        ...
        if elf_filename:
            loadcmds = list(get_loadcmds(elf_filename))
