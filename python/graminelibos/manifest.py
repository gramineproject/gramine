...
class Manifest:
    ...
    def expand_all_trusted_files(self, chroot=None):
        ...
        Raises:
            graminelibos.ManifestError: There was an error with the format of some trusted files in the manifest
        ...
        for data in self['sgx']['trusted_files']:
            for tf in TrustedFile.from_manifest(data, chroot=chroot).expand_directory():
                if tf.uri in trusted_files:
                    # On duplicate entries, pick the one that is already measured, preferring
                    # existing one to avoid dict insertion. Accept double (matching) sha256
                    # deduplicating them, and error out on conflicting measurement.
                    tf_old = trusted_files[tf.uri]
                    if tf_old.sha256 is not None:
                        if tf.sha256 is not None and tf.sha256 != tf_old.sha256:
                            raise ManifestError(
                                f'Two different sha256 values ({tf2.sha256} and {tf.sha256}) '
