...
- ``size2`` is an optional size for the "in" direction of copy. The same rules
  apply to ``size2`` as for ``size``. If ``size2`` is specified, then value in
  ``size`` is used for the "out" copy direction and value in ``size2`` is used
  for the "in" copy direction (``size2`` makes sense only if ``type ==
  "inout"``).
