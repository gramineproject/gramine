...
::
    ...
    sgx.cpu_features.mpx    = "[autodetect|required|disabled]"
    sgx.cpu_features.pkru   = "[autodetect|required|disabled]"
...
not-security-critical CPU features (currently only AVX, AVX512 and AMX) will be
enabled in the enclave if they are available on the platform. Note that
security-critical CPU features (currently MPX and PKRU) will be disabled.
...
auto-detects the corresponding not-security-critical CPU features on the
platform and enables them if available, regardless of whether the application
uses them or not.

At the technical level, the mapping from CPU features to SGX fields is as
follows:

- ``"autodetect"``: ``SIGSTRUCT.ATTRIBUTEMASK[feature] = 0`` for
  not-security-critical CPU features (currently AVX, AVX512 and AMX). The
  untrusted loader of Gramine sets ``SECS.ATTRIBUTES[feature] = 0`` if it can't
  detect the feature on the platform, and ``SECS.ATTRIBUTES[feature] = 1``
  otherwise.
