...
- ``onlyif = "simple boolean expression"`` allows to condition the sub region
  based on a boolean expression. The only currently supported formats of
  expressions are ``token1 == token2``, ``token1 != token2``,
...
For more examples and complex usages of the IOCTL syntax, refer to the Gramine
