...
creating the enclave but allocated dynamically using `EACCEPT` when Gramine
requests more heap memory. This triggers a page fault (#PF) which is handled by
the Intel SGX driver by `EAUG`ing the page and returning the control back to the
enclave. The enclave now continues from the same `EACCEPT`` instruction (but
