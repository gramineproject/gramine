...
.. glossary::
   ...
   Attestation result
      ...
      The result of evaluation of the :term:`evidence` that was generated by the
      :term:`verifier`. The attestation result is typically in the form of a
      signed token (e.g., a JSON Web Token), and typically includes information
      about the :term:`attester` such as the hash of the public key generated by
      the attester, its identity measurements, etc.

      As a particular example, :term:`Intel Attestation Service` generates the
      attestation result as a JSON report. Other examples include Microsoft
      Azure Attestation and Intel Trust Authority that generate attestation
      results in the form of JSON Web Tokens.

   Attester

      In :term:`attestation`, the attester transfers the :term:`evidence` to the
      :term:`verifier` (in :term:`passport model`). Alternatively, the attester
      transfers the evidence to the :term:`relying party` which then forwards it
      to the :term:`verifier` for verification (in :term:`background check
      model`). The evidence contains claims that describe the attester's
      integrity and trustworthiness, as well as the signature that proves the
      claims.

      As a particular example, the attester is the Gramine SGX enclave. It
      generates the evidence (SGX quote plus additional claims) and sends it
      either directly to the verifier or to the relying party, which then will
      forward it to the verifier (see :term:`IAS` for more details).

   Claim
   Attestation claim

      In :term:`attestation`, the claim is a machine-readable assertion about an
      :term:`attester` that describes security-relevant properties, attributes
      or identifiers that can be included in :term:`evidence`,
      :term:`endorsement` or :term:`attestation result`.

      As a particular example, an Intel SGX quote (evidence) contains a claim
      ``MRENCLAVE = <SHA256 hash of initial enclave state>``. Similarly, a
      policy installed in e.g. Microsoft Azure Attestation (verifier) may
      contain a claim ``MRENCLAVE = <expected SHA256 hash of initial enclave
      state>``. Finally, an attestation result from Microsoft Azure Attestation
      may contain a claim ``IS_MRENCLAVE_EXPECTED = true|false``.

   Background check model
   Background check attestation model

      In :term:`attestation`, background check model refers to an attestation
      flow where the :term:`attester` sends its :term:`attestation evidence` to
      the :term:`relying party` (upon request from relying party). The relying
      party then forwards the attestation evidence to the :term:`verifier` and
      receives back the :term:`attestation result`. In other words, the relying
      party performs the background check of the attester.

      This model can be contrasted with the :term:`passport model`.
   ...
   Attestation endorsement
      ...
      Typically, the :term:`verifier` collects endorsements from
   ...
   Attestation evidence
      ...
      Environment` plus the attester's signature over these claims. The evidence
      must be transferred from the :term:`attester` to the :term:`verifier`. The
      claims must be authenticatable, i.e. they must provide a way to the
      verifier to reason about authenticity of the TEE.
   ...
   Passport attestation model
      ...
      In :term:`attestation`, passport model refers to an attestation flow where
      the :term:`attester` sends its :term:`attestation evidence` directly to
      the :term:`verifier` and receives back the :term:`attestation result`.
      Then, upon request, the attester forwards the attestation result to a
      :term:`relying party` as a "passport" for authentication.
   ...
   Attestation policy
      ...
      party` that specifies how the :term:`evidence` is evaluated by the
      :term:`verifier` against :term:`endorsements<endorsement>`. The policy
      also specifies the output format and set of claims in the
      :term:`attestation result`.

      For example, :term:`IAS` has a hard-coded policy to verify claims about
      the Intel SGX hardware and to produce an attestation result with a claim
      (among others) ``hardware_status = ok|error``. Other verifiers like
      Microsoft Azure Attestation and Intel Trust Authority allow users (relying
      parties) to install their own policies.
   ...
   Relying Party
      ...
      In :term:`attestation`, the relying party receives the :term:`attestation
   ...
   Verifier
      ...
      In :term:`attestation`, the verifier receives the :term:`evidence`
      ...
      As a particular example, :term:`Intel Attestation Service` is the verifier
      (though it only evaluates that the hardware is genuine and up-to-date).
      Other examples include Microsoft Azure Attestation and Intel Trust
      Authority (these two evaluate both the hardware and the software
      trustworthiness).
