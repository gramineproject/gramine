...
.. glossary::
   ...
   Attestation result
      ...
      The result of evaluation of the :term:`attestation evidence` that was
      generated by the :term:`verifier`. The attestation result is typically in
      the form of a signed token (e.g., a JSON Web Token), and typically
      includes information about the :term:`attester` such as the hash of the
      public key generated by the attester, its identity measurements, etc.
   ...
   Attester
      ...
      In :term:`attestation`, the attester transfers the :term:`attestation
      evidence` to the :term:`verifier` (in :term:`passport model`).
      Alternatively, the attester transfers the evidence to the :term:`relying
      party` which then forwards it to the :term:`verifier` for verification (in
      :term:`background check model`). The evidence contains :term:`attestation
   ...
   Attestation claim
      ...
      In :term:`attestation`, the attestation claim is a machine-readable
      assertion about an :term:`attester` that describes security-relevant
      properties, attributes or identifiers that can be included in
      :term:`attestation evidence`, :term:`attestation endorsement` or
   ...
   Endorser
      ...
      :term:`attestation endorsement` to the :term:`verifier`.
   ...
   Attestation policy
      ...
      As another example, :term:`IAS` has a hard-coded policy to verify attestation
      claims about the Intel SGX hardware and to produce an attestation result
      with a claim (among others) ``hardware_status = ok|error``.
   ...
   Verifier
      ...
      In :term:`attestation`, the verifier receives the :term:`attestation
      evidence` directly from the :term:`attester` (in :term:`passport model`)
      or indirectly from the :term:`relying party` (in :term:`background check
      model`), as well as the :term:`attestation endorsements<endorsement>` from
      the :term:`endorsers<endorser>`, and sends the :term:`attestation result`
      to the :term:`relying party`. The verifier evaluates the attestation
      evidence to determine attester's trustworthiness.

      As a particular example, :term:`Microsoft Azure Attestation` and
      :term:`Intel Trust Authority` are verifiers: they evaluate both the
      hardware and the software trustworthiness. Another example would be
      :term:`Intel Attestation Service`, though it only evaluates that the
      hardware is genuine and up-to-date.
