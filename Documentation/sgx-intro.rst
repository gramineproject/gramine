...
.. glossary::
   ...
   Attester
      ...
      receive attestation evidence. The :term:`passport model` has the attester
      transfer the :term:`attestation evidence` to the :term:`verifier`, who
      generates the :term:`attestation result` and transfers it back to the
      attester; then the attester can present this attestation result (the
      attester's "passport") to the relying party. Alternatively, the
      :term:`background check model` has the attester transfer the evidence to
      the :term:`relying party` which then forwards it to the :term:`verifier`
      for verification (the relying party performs a "background check" on the
      attester).
   ...
   Attestation endorsement
      ...
      Target Environments. One example of an endorsement is a manufacturer
   ...
   Verifier
      ...
      attestation evidence. The :term:`passport model` has the attester transfer
      the :term:`attestation evidence` to the :term:`verifier`, who generates
      the :term:`attestation result` and transfers it back to the attester; then
      the attester can present this attestation result (the attester's
      "passport") to the relying party. Alternatively, the :term:`background
      check model` has the attester transfer the evidence to the :term:`relying
      party` which then forwards it to the :term:`verifier` for verification
      (the relying party performs a "background check" on the attester).
