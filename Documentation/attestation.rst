...
the so-called evidence claims, with the most important one being the
"pubkey-hash" claim that contains the hash of the ephemeral public key (in DER
format) generated by the enclavized application. For reasons of standardization,
the OID object and all its sub-objects are encoded in the CBOR data format.
...
One can see that the non-standard OID object directly embeds the SGX quote,
doesn't have the concept of claims and doesn't use the CBOR data format.
