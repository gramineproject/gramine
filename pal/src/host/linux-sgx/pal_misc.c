...
static uint64_t get_tsc_hz_hypervisor(void) {
     ...
     *
     * If we detect QEMU/KVM hypervisor (hypervisor_id = "KVMKVMKVM") or VMWare ("VMwareVMware"),
     * then we assume that leaf 0x40000010 contains virtual TSC frequency in kHz in EAX. We check
     * hypervisor_id because leaf 0x40000010 is not standardized and e.g. Microsoft Hyper-V may use
    ...
    if (memcmp(hypervisor_id, "KVMKVMKVM\0\0\0", 12) && memcmp(hypervisor_id, "VMWareVMWare", 12)) {
        /* only QEMU/KVM and VMWare are known to expose TSC frequency in leaf 0x40000010 */
        return 0;
    }
