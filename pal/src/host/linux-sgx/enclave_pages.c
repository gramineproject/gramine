...
static void* __create_vma_and_merge(void* addr, size_t size, bool is_pal_internal,
    ...
    __atomic_add_fetch(&g_allocated_pages, allocated / g_page_size, __ATOMIC_SEQ_CST);
...
int free_enclave_pages(void* addr, size_t size) {
    ...
    __atomic_sub_fetch(&g_allocated_pages, freed / g_page_size, __ATOMIC_SEQ_CST);
