...
int pal_thread_init(void* tcbptr) {
    ...
    __atomic_store_n(&tcb->start_status, 0, __ATOMIC_RELAXED);
    ...
    ret = 0;
out:
    __atomic_store_n(&tcb->start_status, ret, __ATOMIC_RELAXED);
    return ret;
}
...
int clone_thread(void) {
    ...
    while ((ret = __atomic_load_n(&tcb->start_status, __ATOMIC_RELAXED)) == 1)
        CPU_RELAX();
