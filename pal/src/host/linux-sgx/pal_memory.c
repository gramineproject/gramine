...
extern int64_t g_allocated_pages;
...
int _PalVirtualMemoryProtect(void* addr, uint64_t size, pal_prot_flags_t prot) {
    ...
    static int64_t at_cnt = 0;
    int64_t t = 0;
    if (__atomic_compare_exchange_n(&at_cnt, &t, 1, /*weak=*/false, __ATOMIC_SEQ_CST,
                                    __ATOMIC_RELAXED))
...
uint64_t _PalMemoryAvailableQuota(void) {
    return (g_pal_linuxsgx_state.heap_max - g_pal_linuxsgx_state.heap_min) -
           __atomic_load_n(&g_allocated_pages, __ATOMIC_SEQ_CST) * g_page_size;
}
