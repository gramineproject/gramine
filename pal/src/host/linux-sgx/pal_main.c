...
static int print_warnings_on_insecure_configs(PAL_HANDLE parent_process) {
    ...
    bool memfaults_wo_exinfo_allowed = g_pal_linuxsgx_state.memfaults_wo_exinfo_allowed;
    ...
    if (!verbose_log_level && !sgx_debug && !use_cmdline_argv && !use_host_env && !disable_aslr &&
            ...
            !encrypted_files_keys && !memfaults_wo_exinfo_allowed) {
    ...
    if (memfaults_wo_exinfo_allowed)
        log_always("  - sgx.insecure__allow_memfaults_wo_exinfo    "
...
noreturn void pal_linux_main(void* uptr_libpal_uri, size_t libpal_uri_len, void* uptr_args,
     ...
     *       to deliver memory faults instead of terminating; remove this in v1.8 */
    ret = toml_bool_in(g_pal_public_state.manifest_root, "sgx.insecure__allow_memfaults_wo_exinfo",
                       /*defaultval=*/false, &g_pal_linuxsgx_state.memfaults_wo_exinfo_allowed);
    if (ret < 0) {
        log_error("Cannot parse 'sgx.insecure__allow_memfaults_wo_exinfo'");
        ocall_exit(1, /*is_exitgroup=*/true);
    }
