...
/* FIXME: SECS struct must also contain fields ISVEXTPRODID and ISVFAMILYID but they are not
 *        described in Intel SDM (Vol. 3, Section 34.7, Table 34-2) which must be a bug */
typedef struct {
    uint64_t             size;
    uint64_t             base;
