...
int ocall_lstat(const char* link_path, struct stat* buf) {
    int retval = 0;
    size_t link_size = link_path ? strlen(link_path) + 1 : 0;
    struct ocall_lstat* ocall_lstat_args;

    void* old_ustack = sgx_prepare_ustack();
    ocall_lstat_args = sgx_alloc_on_ustack_aligned(sizeof(*ocall_lstat_args),
                                                   alignof(*ocall_lstat_args));
    if (!ocall_lstat_args) {
        sgx_reset_ustack(old_ustack);
        return -EPERM;
    }

    void* untrusted_readlink_path = sgx_copy_to_ustack(link_path, link_size);
    if (!untrusted_readlink_path) {
        sgx_reset_ustack(old_ustack);
        return -EPERM;
    }

    COPY_VALUE_TO_UNTRUSTED(&ocall_lstat_args->link_path, untrusted_readlink_path);

    do {
        retval = sgx_exitless_ocall(OCALL_LSTAT, ocall_lstat_args);
    } while (retval == -EINTR);
    ...
    if (!retval) {
        if (!sgx_copy_to_enclave(buf, sizeof(*buf), 
                                 &ocall_lstat_args->stat, sizeof(ocall_lstat_args->stat))) {
            retval = -EPERM;
        }
        /* TODO: sanitize `buf`, e.g. that `buf->st_size >= 0` */
    }
...
int ocall_readlink(const char* link_path, char* buf, size_t buf_sz, ssize_t* ret_len) {
    int retval = 0;
    size_t link_size = link_path ? strlen(link_path) + 1 : 0;
    struct ocall_readlink* ocall_readlink_args;

    void* old_ustack = sgx_prepare_ustack();
    ocall_readlink_args = sgx_alloc_on_ustack_aligned(sizeof(*ocall_readlink_args),
                                                      alignof(*ocall_readlink_args));
    if (!ocall_readlink_args) {
        sgx_reset_ustack(old_ustack);
        return -EPERM;
    }

    void* untrusted_readlink_path = sgx_copy_to_ustack(link_path, link_size);
    void* untrusted_buf = sgx_alloc_on_ustack(buf_sz);
    if (!untrusted_readlink_path || !untrusted_buf) {
        sgx_reset_ustack(old_ustack);
        return -EPERM;
    }
    COPY_VALUE_TO_UNTRUSTED(&ocall_readlink_args->link_path, untrusted_readlink_path);
    COPY_VALUE_TO_UNTRUSTED(&ocall_readlink_args->buf, untrusted_buf);
    COPY_VALUE_TO_UNTRUSTED(&ocall_readlink_args->buf_sz, buf_sz);

    do {
        retval = sgx_exitless_ocall(OCALL_READLINK, ocall_readlink_args);
    } while (retval == -EINTR);

    if (retval < 0 && retval != -EACCES && retval != -EFAULT && retval != -EINVAL &&
        retval != -EIO && retval != -ELOOP && retval != -ENAMETOOLONG && retval != -ENOENT &&
        retval != -ENOMEM && retval != -ENOTDIR) {
        retval = -EPERM;
        goto out;
    }

    if (!sgx_copy_to_enclave(buf, buf_sz, 
                             ocall_readlink_args->buf, ocall_readlink_args->buf_sz)) {
        retval = -EPERM;
        goto out;
    }
    if (ret_len != NULL)
        *ret_len = COPY_UNTRUSTED_VALUE(&ocall_readlink_args->ret_len);
...
int ocall_link(const char* oldpath, const char* newpath, bool is_soft_link) {
    ...
    size_t old_size = oldpath ? strlen(oldpath) + 1 : 0;
    size_t new_size = newpath ? strlen(newpath) + 1 : 0;
    struct ocall_link* ocall_link_args;

    void* old_ustack = sgx_prepare_ustack();
    ocall_link_args = sgx_alloc_on_ustack_aligned(sizeof(*ocall_link_args),
                                                  alignof(*ocall_link_args));
    if (!ocall_link_args) {
        sgx_reset_ustack(old_ustack);
        return -EPERM;
    }
    ...
    COPY_VALUE_TO_UNTRUSTED(&ocall_link_args->oldpath, untrusted_oldpath);
    COPY_VALUE_TO_UNTRUSTED(&ocall_link_args->newpath, untrusted_newpath);
    COPY_VALUE_TO_UNTRUSTED(&ocall_link_args->is_soft_link, is_soft_link);

    do {
        retval = sgx_exitless_ocall(OCALL_LINK, ocall_link_args);
    } while (retval == -EINTR);

    if (retval < 0 && retval != -EACCES && retval != -EDQUOT && retval != -EEXIST &&
        retval != -EFAULT && retval != -EIO && retval != -ELOOP && retval != -EMLINK &&
        retval != -ENAMETOOLONG && retval != -ENOENT && retval != -ENOMEM &&
        retval != -ENOSPC && retval != -ENOTDIR &&  retval != -EPERM && retval != -EROFS &&
        retval != -EXDEV && retval != -EBADF &&
        retval != -EBUSY && retval != -EINVAL && retval != -EISDIR && retval != -ENOTEMPTY) {
        retval = -EPERM;
    }
