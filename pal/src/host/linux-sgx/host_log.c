...
static void print_to_fd(int fd, const char* prefix, const char *file, const char *func, uint64_t
                        line, const char* fmt, va_list ap) {
    struct print_buf buf = INIT_PRINT_BUF_ARG(buf_write_all, &fd);

    if (LOG_LEVEL_DEBUG <= g_host_log_level)
        buf_printf(&buf, "(%s:%ld:%s) ", file, line, func);
    if (prefix)
        buf_puts(&buf, prefix);
