...
static void read_append(const char* file_path) {
    ...
    write_fd(file_path, fd, buf1 + size / 2, size / 2);
    printf("second write(%s) OK\n", file_path);
    bytes_read = read(fd, buf2, size);
    if (bytes_read != 0)
        fatal_error("Read after append does not indicate EOF\n");
    ...
    /* test 2: open file created previously (it must contain buf1), then read (this should return
     * buf1 as there was no write/append operation yet, so file pos is zero), then reset the file
     * pos and append buf1 (this should result in buf1 + buf1 contents of the file) */
    printf("TEST 2 (%s)\n", file_path);
    ...
    bytes_read = read(fd, buf2, size);
    if (bytes_read != 0)
        fatal_error("Second read does not indicate EOF\n");
    printf("second read(%s) OK\n", file_path);
    seek_fd(file_path, fd, 0, SEEK_SET);
    printf("seek(%s) OK\n", file_path);
     ...
     * file pos and write buf1 (must overwrite contents, so file size must stay the same), then
     * change to append mode, reset file pos and write buf1 (must append, so size will increase) */
    printf("TEST 3 (%s)\n", file_path);
