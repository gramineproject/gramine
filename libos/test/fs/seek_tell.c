...
#define VERIFYU(msg, expected, got) do {                    \
    ...
    uint64_t __r = (got);                                   \
    if (__l != __r) {                                       \
        fatal_error("%s:%d %s (expected %zu, got %zu)",     \
                    __func__, __LINE__, msg, __l, __r);     \
    }                                                       \
} while(0)

static void seek_input_fd(const char* path, size_t size) {
    int f = open_input_fd(path);

    seek_fd(path, f, 0, SEEK_SET);
    off_t pos = tell_fd(path, f);
    VERIFYU("tell position mismatch", 0, (size_t)pos);

    seek_fd(path, f, 0, SEEK_END);
    pos = tell_fd(path, f);
    VERIFYU("tell position mismatch", size, (size_t)pos);
...
static void seek_input_stdio(const char* path, size_t size) {
    FILE* f = open_input_stdio(path);

    seek_stdio(path, f, 0, SEEK_SET);
    off_t pos = tell_stdio(path, f);
    VERIFYU("ftell position mismatch", 0, (size_t)pos);

    seek_stdio(path, f, 0, SEEK_END);
    pos = tell_stdio(path, f);
    VERIFYU("ftell position mismatch", size, (size_t)pos);
...
static void seek_output_fd(const char* path, size_t size) {
    ...
    size_t pos_over = pos * 2;
    seek_fd(path, f, pos_over, SEEK_SET);
    pos = tell_fd(path, f);
    VERIFYU("tell position mismatch", pos_over, (size_t)pos);
...
static void seek_output_stdio(const char* path, size_t size) {
    uint8_t buf[EXTEND_SIZE + 1] = {1};
    ...
    pos = tell_stdio(path, f);
    VERIFYU("ftell position mismatch", pos_over, (size_t)pos);
...
int main(int argc, char* argv[]) {
    ...
    size_t argv1_size = file_size(argv[1]);
    size_t argv2_size = file_size(argv[2]);
    size_t argv3_size = file_size(argv[3]);
