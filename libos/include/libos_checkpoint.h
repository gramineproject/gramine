...
static inline void checkpoint_rlock(const char *caller, unsigned int line)
{
    ...
    log_error("%s @ %u: after RLOCK: %d", caller, line, n);
#endif
}
...
static inline void checkpoint_runlock(const char *caller, unsigned int line)
{
    ...
    log_error("%s @ %u: after RUNLOCK: %d", caller, line, n);
#endif
}
...
#ifdef DEBUG_CHECKPOINT_LOCK
    log_error("%s @ %u: at WLOCK: %d", caller, line, __atomic_load_n(&checkpoint_lock_ctr, __ATOMIC_ACQUIRE));
#endif
}
...
#ifdef DEBUG_CHECKPOINT_LOCK
    log_error("%s @ %u: at WUNLOCK: %d", caller, line, __atomic_load_n(&checkpoint_lock_ctr, __ATOMIC_ACQUIRE));
#endif
}
