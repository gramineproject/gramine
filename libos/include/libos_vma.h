 ...
 * [`g_pal_public_state->private_user_address_start`,
...
/* First tries to bookkeep in the range
 * [`g_pal_public_state->private_user_address_start`,`aslr_addr_top`) and if it fails calls
 * `bkeep_mmap_any`. `aslr_addr_top` is a value randomized on each program run. */
