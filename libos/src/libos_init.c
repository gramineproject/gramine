...
// Warning: not side-channel-secure! But we don't need this property in the current callsites.
...
int create_pipe(char* name, char* uri, size_t size, PAL_HANDLE* hdl, bool use_vmid_for_name) {
    ...
    while (true) {
        if (use_vmid_for_name) {
            ...
            /* No need for a side-channel-secure hex conversion, this name is known to the untrusted
             * host anyways. */
