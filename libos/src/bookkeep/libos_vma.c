...
int init_vma(void) {
    ...
    g_aslr_addr_top = g_pal_public_state->private_user_address_end;

    if (!g_pal_public_state->disable_aslr) {
        /* Inspired by: https://elixir.bootlin.com/linux/v5.6.3/source/arch/x86/mm/mmap.c#L80 */
        size_t gap_max_size = (g_pal_public_state->private_user_address_end -
                               g_pal_public_state->private_user_address_start) / 6 * 5;
...
int bkeep_mmap_any(size_t length, int prot, int flags, struct libos_handle* file, uint64_t offset,
    ...
    return bkeep_mmap_any_in_range(g_pal_public_state->private_user_address_start,
...
int bkeep_mmap_any_aslr(size_t length, int prot, int flags, struct libos_handle* file,
    ...
    int ret;
    ret = bkeep_mmap_any_in_range(g_pal_public_state->private_user_address_start, g_aslr_addr_top,
                                  length, prot, flags, file, offset, comment, ret_val_ptr);
    if (ret >= 0) {
        return ret;
    }
