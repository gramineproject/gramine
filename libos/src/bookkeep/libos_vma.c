...
static int reload_mmaped_from_file_all(uintptr_t begin, uintptr_t end, struct libos_handle* hdl) {
    ...
    for (size_t i = 0; i < count && !ret; i++) {
        ...
        if (!(vma_info->prot & PROT_WRITE)) {
            /* make the area writable so that it can be reloaded */
            if (PalVirtualMemoryProtect((void*)read_begin, size,
                    LINUX_PROT_TO_PAL(vma_info->prot | PAL_PROT_WRITE, vma_info->flags)) < 0) {
                BUG();
            }
        }
        ...
        if (!(vma_info->prot & PROT_WRITE)) {
            ...
            if (PalVirtualMemoryProtect((void*)read_begin, size,
                    LINUX_PROT_TO_PAL(vma_info->prot, vma_info->flags)) < 0) {
                BUG();
            }
        }
    }
