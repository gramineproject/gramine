...
int init_brk_region(void* brk_start, size_t data_segment_size) {
    ...
    if (brk_start && brk_start <= g_pal_public_state->memory_address_end
        ...
        ret = bkeep_mmap_fixed(brk_start, brk_max_size, PROT_NONE,
                               MAP_FIXED_NOREPLACE | VMA_UNMAPPED, /*file=*/NULL, /*offset=*/0,
                               "heap", /*overwriting=*/NULL);
        if (ret == -EEXIST) {
...
void* libos_syscall_brk(void* _brk) {
    ...
    if (brk < brk_region.brk_start) {
        ...
        if (size) {
            if (bkeep_mmap_fixed(brk_aligned, brk_region.brk_end - brk_aligned, PROT_NONE,
                                 MAP_FIXED | VMA_UNMAPPED, /*file=*/NULL, /*offset=*/0, "heap",
                                 /*overwriting=*/NULL)) {
                goto out;
            }
    ...
    if (bkeep_mmap_fixed(brk_current, size, PROT_READ | PROT_WRITE,
                         MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED, /*file=*/NULL, /*offset=*/0,
                         "heap", /*overwriting=*/NULL) < 0) {
        goto out;
    }
