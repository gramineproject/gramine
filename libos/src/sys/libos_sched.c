...
long libos_syscall_sched_setaffinity(pid_t pid, unsigned int user_mask_size,
    ...
    /* allocate temporary CPU mask on stack for the common case of platforms with <= 8192 CPUs; we
    ...
    if (cpu_mask_size <= 1024) {
        /* fast path: allocate on stack if the platform has <= 8192 CPUs */
        cpu_mask_on_heap = false;
        cpu_mask = __alloca(cpu_mask_size);
    } else {
        /* slow path: allocate on heap if the platform has > 8192 CPUs */
