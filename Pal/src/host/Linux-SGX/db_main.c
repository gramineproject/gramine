...
noreturn void pal_linux_main(char* uptr_libpal_uri, size_t libpal_uri_len,
                             uintptr_t uptr_libpal_addr, char* uptr_args, size_t args_size,
                             char* uptr_env, size_t env_size, struct pal_sec* uptr_sec_info) {
    /* Our arguments are coming directly from the urts. We are responsible to check them. */
    int ret;

    /* Check that pointer to PAL binary points inside the SGX enclave (we heuristically check the
     * first 256B where the ELF header and program headers reside, this should be enough to make
     * sure this is the actual ELF file and not some gadget bytes) */
    if (!sgx_is_completely_within_enclave((void*)uptr_libpal_addr, /*size=*/256)) {
        log_error("PAL binary is not located inside the SGX enclave");
        ocall_exit(1, /*is_exitgroup=*/true);
    }

    /* Relocate PAL and populate g_pal_map */
    ret = setup_pal_binary(uptr_libpal_addr, &g_pal_map);
    if (ret < 0) {
