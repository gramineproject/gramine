...
static int initialize_enclave(struct pal_enclave* enclave, const char* manifest_to_measure) {
    ...
    sigfile = DO_SYSCALL(open, sig_path, O_RDONLY | O_CLOEXEC, 0);
    if (sigfile < 0) {
    ...
    token = DO_SYSCALL(open, token_path, O_RDONLY | O_CLOEXEC, 0);
    if (token < 0) {
        log_error(
    ...
    ret = read_enclave_token(token, &enclave_token);
    if (ret < 0) {
...
out:
    ...
    if (sigfile >= 0)
        DO_SYSCALL(close, sigfile);
    if (token >= 0)
        DO_SYSCALL(close, token);
    free(sig_path);
    free(token_path);
    return ret;
}
