...
typedef struct {
    ...
    union {
        /* Common filed for accessing underlying host fd. See also `PAL_HANDLE_FD_READABLE`. */
        struct {
            PAL_IDX fd;
        } generic;
 ...
 * writable respectively. If non of these is set, then the handle has no host-level fd. */
