 ...
 *     name of a sub-region where the actual size is stored. If the `size2` field is additionally
 *     specified, then value in `size` is used for the "out" copy direction and value in `size2` is
 *     used for the "in" copy direction (`size2` makes sense only if `type` == "inout").
...
struct sub_region {
    ...
    ssize_t size2;            /* may be dynamically determined from another sub-region */
    char* size2_name;         /* needed if "size2" sub region is defined after this sub region */
    uint64_t size2_name_hash; /* needed if "size2" sub region is defined after this sub region */
    bool size2_name_need_free;/* false if "size2_name" ownership was transfered to the cache */
...
struct cached_sub_region_info {
    ...
    ssize_t size2; /* -1 if dynamically determined based on name, then use size2_name */
    char* size2_name;
    uint64_t size2_name_hash;
    ssize_t align;
    ssize_t unit;
    ssize_t adjust;
};
...
static int collect_sub_regions(toml_array_t* root_toml_array, void* root_encl_addr,
    ...
    while (mem_region_idx < mem_regions_cnt) {
        ...
        for (int i = 0; i < toml_array_nelem(cur_mem_region->toml_array); i++) {
            ...
            toml_raw_t sub_region_size2_raw  = cached ? cached->size2_raw
                                                      : toml_raw_in(sub_region_info, "size2");
            ...
            /* logic for "size2" is exactly the same as for "size1", see above */
            cur_sub_region->size2 = -1;
            cur_sub_region->size2_name = NULL;
            cur_sub_region->size2_name_hash = 0;
            cur_sub_region->size2_name_need_free = false;
            if (cached && cached->size2_name) {
                cur_sub_region->size2_name = cached->size2_name;
                cur_sub_region->size2_name_hash = cached->size2_name_hash;

                ssize_t val = -1;
                ret = get_sub_region_value(sub_regions, sub_regions_cnt - 1,
                                           cur_sub_region->size2_name,
                                           cur_sub_region->size2_name_hash, &val);
                if (ret < 0 && ret != -ENOENT) {
                    goto out;
                }
                cur_sub_region->size2 = val;
            } else if (cached && cached->size2 > 0) {
                cur_sub_region->size2 = cached->size2;
            } else if (sub_region_size2_raw) {
                ret = toml_rtos(sub_region_size2_raw, &cur_sub_region->size2_name);
                if (ret == 0) {
                    cur_sub_region->size2_name_hash = hash(cur_sub_region->size2_name);
                    cur_sub_region->size2_name_need_free = true;
                    ...
                    ret = toml_rtoi(sub_region_size2_raw, &cur_sub_region->size2);
                    if (ret < 0 || cur_sub_region->size2 <= 0) {
                        log_error("Invalid deep-copy syntax ('size2' of a deep-copy "
                                  "sub-entry must be a TOML string or a positive number)");
                        ret = -EINVAL;
                        goto out;
                    }
            ...
            if (cur_sub_region->type != COPY_INOUT_ENCLAVE &&
                    (cur_sub_region->size2 >= 0 || cur_sub_region->size2_name)) {
                log_error("Invalid deep-copy syntax ('size2' can be used only with 'type = inout' "
                          "in a deep-copy sub-entry)");
                ret = -EINVAL;
                goto out;
            }
            ...
            if (!cached) {
                ...
                new_cached->size2_raw       = sub_region_size2_raw;
                ...
                new_cached->size2_name      = cur_sub_region->size2_name;
                new_cached->size2_name_hash = cur_sub_region->size2_name_hash;
                ...
                /* check if "size"/"size2" are static (and positive) -> cache them */
                new_cached->size = -1;
                new_cached->size2 = -1;
                if (cur_sub_region->size > 0 && !cur_sub_region->size_name)
                    new_cached->size = cur_sub_region->size;
                if (cur_sub_region->size2 > 0 && !cur_sub_region->size2_name)
                    new_cached->size2 = cur_sub_region->size2;
            ...
            if (cur_sub_region->size2 >= 0) {
                cur_sub_region->size2 *= cur_sub_region->unit;
                cur_sub_region->size2 += cur_sub_region->adjust;
            }

            ssize_t max_sub_region_size = MAX(cur_sub_region->size, cur_sub_region->size2);
            cur_encl_addr += cur_sub_region->type == COPY_PTR_ENCLAVE ? sizeof(void*) :
                                 (uintptr_t)max_sub_region_size;
        }

        /* iterate through collected pointer/array sub regions and add corresponding mem regions
         * (note that "size2" field is not relevant here because these are only COPY_PTR regions) */
...
out:
    ...
    for (int i = 0; i < sub_regions_cnt; i++) {
        ...
        log_error("===== sub_region[%d]: '%s', size=%ld, size2=%ld, addr=%p, value=%u",
                  i, sub_regions[i].name, sub_regions[i].size, sub_regions[i].size2,
    ...
    for (int i = 0; i < sub_regions_cnt; i++) {
        ...
        if (sub_regions[i].size2_name_need_free)
            free(sub_regions[i].size2_name);
        sub_regions[i].name = NULL;
        sub_regions[i].size_name = NULL;
        sub_regions[i].size2_name = NULL;
    }
    return ret;
}
...
static void copy_sub_regions_to_enclave(struct sub_region* sub_regions, int sub_regions_cnt) {
    for (int i = 0; i < sub_regions_cnt; i++) {
        /* direction of copy is "in": if "size2" was specified in ioctl, use it instead of "size" */
        ssize_t copy_size = sub_regions[i].size2 >= 0 ? sub_regions[i].size2 : sub_regions[i].size;
        if (copy_size <= 0 || !sub_regions[i].encl_addr)
            continue;

        if (sub_regions[i].type == COPY_IN_ENCLAVE || sub_regions[i].type == COPY_INOUT_ENCLAVE)
            memcpy(sub_regions[i].encl_addr, sub_regions[i].untrusted_addr, copy_size);
    }
}
...
int _DkDeviceIoControl(PAL_HANDLE handle, unsigned int cmd, uint64_t arg) {
    ...
    for (int i = 0; i < sub_regions_cnt; i++)
        untrusted_size += MAX(sub_regions[i].size, sub_regions[i].size2) + sub_regions[i].align;
