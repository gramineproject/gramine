 ...
 * `size_mult` is passed, then size qualifier is stored while reading the integer. For example on a
 * single-core machine, calling this function on `/sys/devices/system/cpu/online` with count == true
 * will return 1 and 0 with count == false. Returns UNIX error code on failure.
